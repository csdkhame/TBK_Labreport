...
<div class="" >
 <div class="panel panel-default" style="margin-bottom: 10px;">
  ...
  <ul class="dropdown-menu" >
    <li ><a style="line-height: 2.428571;cursor:pointer;" onclick="update_lab_meet('<?=$tb_md1[id]?>',1,'<?=$tb_md1[invoice]?>','<? echo $server; ?>','<?=$_GET[user];?>','<?=$tb_md1[orderid];?>');"  >รับแขก</a></li>
...
onclick="viewDetail('<?=$tb_md1[id];?>', 'งาน <?=$title_area;?>','<?=$server;?>');" >
          ...
          <td>
            ...
            if($tb_md1[lab_approve] != "0") {
              ?>
              <img src="iconstatus/all/delete.png" width="30" height="30" align="middle" onclick="update_lab_meet('<?=$tb_md1[id]?>',1,'<?=$tb_md1[invoice]?>','<? echo $server; ?>','<?=$_GET[user];?>','<?=$tb_md1[orderid]?>');" style="cursor:pointer;" />
              <?php
            }?></td>
          <td>
            <span data-placement="top" data-toggle="tooltip" title="View">
...
update_lab_approve('<?=$tb_md2[id];?>','1','<?=$tb_md2[invoice];?>','<?=$server;?>','<?=$_GET[user];?>');" ><div><span class="glyphicon glyphicon-briefcase"></span></div></button></span>
    <?  }
            ...
            <span data-placement="top" data-toggle="tooltip" title="View">
              ...
              <img src="iconstatus/all/accept.png" width="30" height="30" align="middle" onclick="update_lab_meet('<?=$tb_md2[id]?>',0,'<?=$tb_md2[invoice]?>','<?=$_GET[user];?>','<?=$tb_md2[invoice]?>','<?=$tb_md2[orderid]?>');" style="cursor:pointer;" />&nbsp;&nbsp;&nbsp;
            <span data-placement="top" data-toggle="tooltip" title="View">
